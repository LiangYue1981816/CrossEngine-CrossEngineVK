...
intermediate_type parse_intermediate(const uint32_t* words, const size_t num_words) {
	std::shared_ptr<spv_context_t> context(spvContextCreate(SPV_ENV_VULKAN_1_0), spvContextDestroy);
	assert(context);

	intermediate_type intermediate;
	spv_diagnostic diagnostic;
	spvBinaryParse(context.get(), &intermediate, words, num_words, &parsed_header, &parsed_instruction, &diagnostic);
	return intermediate;
}

intermediate_type parse_intermediate(std::istream &stream) {
	const std::string content(std::istreambuf_iterator<char>(stream.rdbuf()), std::istreambuf_iterator<char>());
	return parse_intermediate((uint32_t *)content.c_str(), content.size() / sizeof(uint32_t));
}
...
module_type parse(const uint32_t* words, const size_t num_words) {
	return convert(parse_intermediate(words, num_words));
}
...
module_type parse(const uint32_t* words, const size_t num_words) {
	return internal::parse(words, num_words);
}
