...
void CreateBuffer(void)
{
	...
	ptrVertexBuffer->Create(vertexBufferSize, vertexBuffer.data(), FALSE, CrossEngine::VERTEX_ATTRIBUTE_POSITION | CrossEngine::VERTEX_ATTRIBUTE_TEXCOORD0, VERTEX_BUFFER_BINDING);
		...
		ptrCommandBuffers[indexView]->BeginPrimary(VK_COMMAND_BUFFER_USAGE_SIMULTANEOUS_USE_BIT);
		{
			ptrCommandBuffers[indexView]->CmdBeginRenderPass(ptrFrameBuffers[indexView], ptrRenderPass, VK_SUBPASS_CONTENTS_INLINE);
			{
				...
				ptrCommandBuffers[indexView]->CmdBindPipelineGraphics(ptrPipeline);
				{
					...
					ptrCommandBuffers[indexView]->CmdBindDescriptorSetGraphics(ptrDescriptorSetTransform);
					ptrCommandBuffers[indexView]->CmdBindDescriptorSetGraphics(ptrDescriptorSetColor);
