...
private:
	typedef typename detail::DefaultVoidToVoid<RetType>::type DesiredRetType;
	typedef DesiredRetType (*StaticFunctionPtr)(Types...);
	typedef RetType (*UnvoidStaticFunctionPtr)(Types...);
	typedef RetType (detail::GenericClass::*GenericMemFn)(Types...);
	typedef detail::ClosurePtr<GenericMemFn, StaticFunctionPtr, UnvoidStaticFunctionPtr> ClosureType;
	ClosureType m_Closure;
public:
	// Typedefs to aid generic programming
	...
	// Binding to non-const member functions
	template < class X, class Y >
	FastDelegate(Y *pthis, DesiredRetType (X::* function_to_bind)(Types...) ) {
		m_Closure.bindmemfunc(detail::implicit_cast<X*>(pthis), function_to_bind); }
	template < class X, class Y >
	inline void bind(Y *pthis, DesiredRetType (X::* function_to_bind)(Types...)) {
		m_Closure.bindmemfunc(detail::implicit_cast<X*>(pthis), function_to_bind);	}
	// Binding to const member functions.
	template < class X, class Y >
	FastDelegate(const Y *pthis, DesiredRetType (X::* function_to_bind)(Types...) const) {
		m_Closure.bindconstmemfunc(detail::implicit_cast<const X*>(pthis), function_to_bind);	}
	template < class X, class Y >
	inline void bind(const Y *pthis, DesiredRetType (X::* function_to_bind)(Types...) const) {
		m_Closure.bindconstmemfunc(detail::implicit_cast<const X *>(pthis), function_to_bind);	}
	// Static functions. We convert them into a member function call.
	// This constructor also provides implicit conversion
	FastDelegate(DesiredRetType (*function_to_bind)(Types...) ) {
		bind(function_to_bind);	}
	// for efficiency, prevent creation of a temporary
	void operator = (DesiredRetType (*function_to_bind)(Types...) ) {
		bind(function_to_bind);	}
	inline void bind(DesiredRetType (*function_to_bind)(Types...)) {
		m_Closure.bindstaticfunc(this, &FastDelegate::InvokeStaticFunction, 
			function_to_bind); }
	// Invoke the delegate
	RetType operator() (Types... params) const {
...
private:	// Invoker for static functions
	RetType InvokeStaticFunction(Types... params) const {
...
template <class RetType, class X, class Y, class... Types>
FastDelegate<RetType, Types...> MakeDelegate(Y* x, RetType(X::*func)(Types... params)) {
	return FastDelegate<RetType, Types...>(x, func);
}

template <class RetType, class X, class Y, class... Types>
FastDelegate<RetType, Types...> MakeDelegate(Y* x, RetType(X::*func)(Types... params) const) {
	return FastDelegate<RetType, Types...>(x, func);
}
