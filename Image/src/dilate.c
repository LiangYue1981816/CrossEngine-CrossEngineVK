...
static void * dilate8bit(IMAGE *pimgsrc, IMAGE *pimgdst, IMAGE *pimgwrk, int amount);
static void * dilate24bit(IMAGE *pimgsrc, IMAGE *pimgdst, IMAGE *pimgwrk, int amount);
...
EXPORT_C int IMAGE_Dilate(IMAGE * const pimgsrc, IMAGE * const pimgdst, int const namount)
{
	...
	case  8: dilate8bit(pimgsrc, pimgdst, &imgwrk, namount); break;
	case 24: dilate24bit(pimgsrc, pimgdst, &imgwrk, namount); break;
	...
	for (y = 0; y < height; y++) {
		...
		for (x = 0; x < width; x++) {
			...
			if (pixel < amount) {
				...
				index = -1;
				darkest = 0xff;
				if (darkest > ppixel_gry_top[x-1]) { darkest = ppixel_gry_top[x-1]; index = 00; }
				if (darkest > ppixel_gry_top[x  ]) { darkest = ppixel_gry_top[x  ]; index = 01; }
				if (darkest > ppixel_gry_top[x+1]) { darkest = ppixel_gry_top[x+1]; index = 02; }
				if (darkest > ppixel_gry_ths[x-1]) { darkest = ppixel_gry_ths[x-1]; index = 10; }
				if (darkest > ppixel_gry_ths[x  ]) { darkest = ppixel_gry_ths[x  ]; index = 11; }
				if (darkest > ppixel_gry_ths[x+1]) { darkest = ppixel_gry_ths[x+1]; index = 12; }
				if (darkest > ppixel_gry_btm[x-1]) { darkest = ppixel_gry_btm[x-1]; index = 20; }
				if (darkest > ppixel_gry_btm[x  ]) { darkest = ppixel_gry_btm[x  ]; index = 21; }
				if (darkest > ppixel_gry_btm[x+1]) { darkest = ppixel_gry_btm[x+1]; index = 22; }
				if (-1 == index) continue;
